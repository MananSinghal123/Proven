...
export function Navbar() {
    ...
    return (_jsx("nav", { className: "fixed top-0 w-full bg-white dark:bg-[#0A0A0A] border-b-4 border-black dark:border-white z-50", children: _jsxs("div", { className: "max-w-7xl mx-auto px-6 h-20 flex items-center justify-between", children: [_jsxs(Link, { to: "/", className: "flex items-center gap-2", children: [_jsx(ProvenLogo, { size: 24, className: "shrink-0" }), _jsx("span", { className: "text-3xl font-black uppercase tracking-tighter text-black dark:text-white", children: "Proven" })] }), _jsx("div", { className: "hidden md:flex items-center gap-2 font-bold tracking-tight", children: links.map(({ path, label }) => {
                        ...
                        return (_jsx(Link, { to: path, className: `px-3 py-1.5 border-2 transition-colors ${isActive
                                ...
                                : 'border-transparent hover:bg-black hover:text-[#DFFF00] hover:border-black dark:hover:bg-white dark:hover:text-black dark:hover:border-white'}`, children: label }, path));
                    }) }), _jsxs("div", { className: "flex items-center gap-3", children: [_jsx("button", { onClick: toggle, className: "w-10 h-10 border-4 border-black dark:border-white flex items-center justify-center bg-white dark:bg-[#0A0A0A] hover:bg-[#DFFF00] dark:hover:bg-[#DFFF00] dark:hover:text-black transition-colors", "aria-label": "Toggle dark mode", children: dark ? _jsx(Sun, { className: "w-4 h-4 stroke-[2.5]" }) : _jsx(Moon, { className: "w-4 h-4 stroke-[2.5]" }) }), isHome ? (_jsx(Link, { to: "/launch", className: "bg-[#DFFF00] font-bold uppercase border-4 border-black px-6 py-2 text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-1 hover:-translate-x-1 hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all active:translate-y-0 active:translate-x-0 active:shadow-[0px_0px_0px_0px_rgba(0,0,0,1)]", children: "Launch App" })) : (_jsx(ConnectWallet, {}))] })] }) }));
}
